...
//-----------------------------------------------------------------//
//  x.yz     | 11.06.2019 |  FRE: no more entries will be added    //
//           |            |  as the file is revisioned in SVN      //
//-----------------------------------------------------------------//
...
typedef struct
{
  ...
  SHORT       sCCDTemperature;         // CCD temperature in 0.1°C
...
typedef struct
{
  ...
  DWORD       dwGeneralCapsDESC3_1;               // 
  DWORD       dwGeneralCapsDESC3_2;               // 
  DWORD       dwGeneralCapsDESC3_3;               // 
  DWORD       dwGeneralCapsDESC3_4;               // (20)
...
typedef struct
{
  ...
  void          *pSC2IFFunc[20];                                                            // 92 (172 @64bit)
  PCO_APIBuffer strPCOBuf[PCO_BUFCNT]; // Bufferlist                                        // 1692 (1836 @64bit)
  PCO_APIBuffer ZZstrDummyBuf[28-PCO_BUFCNT];     // Bufferlist                                        // 2892 (3084 @64bit)
  ...
  WORD          wXRes;                 // X Resolution in Grabber (CamLink only)            // 2900 (3092 @64bit)
  WORD          wYRes;                 // Y Resolution in Buffer (CamLink only)             // 2902 (3094 @64bit)
  WORD          wPowerCycleFlag;       // This will be set to 0xFFFF in order to forward an eventuall power on state
  DWORD         dwIF_param[5];         // Interface specific parameter                      // 2924 (3116 @64bit)
  ...
  WORD          wInterfacePatchVersion;
  WORD          ZZwDummy[13];                                                               // 2976 (3168 @64bit)
} PCO_APIManagement;
